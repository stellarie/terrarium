...
fertility map live: fewer cores flatten the terrain toward the old uniform sine-grating world,
more cores add lush refugia from a pre-generated pool, and vegetation adapts naturally over the
following ticks without touching any creature or nutrient value.
...
| `smoke.js` | headless smoke test — 121 assertions over thousands of real ticks |
...
onto the hidden landscape, a conserved **nutrient cycle**, a 121-check headless smoke test, and a headless
...
Newest: **body size finally has a genuine food-income advantage, and the concealment mechanic is measurable.** The `sizeGrazeStrength` parameter was raised from 0.6 to 0.8, shifting the bite-rate multiplier range from 0.70×–1.30× to 0.60×–1.40×. The fix was necessary: at 0.6, a small mote's bite rate (0.112) exceeded typical sparse-cell vegetation supply (~0.098), so both size classes took the same bite and large motes paid higher metabolic cost for equal food — selection stayed pinned to the floor. At 0.8 the small-mote bite rate drops to 0.096, just below supply, so large motes gain a consistent advantage in every non-depleted cell without overreaching (0.9 cut too deep, at 0.088, and crashed the hunter tier by starving prey faster than predation could follow). Across two seeds, mean body size drifted from 3.29 → 3.41 — the first upward drift recorded in the arc. A **SORTING signal** accompanied it: the large half of the herd sat on 0.06 fertility-units richer ground on average, suggesting large motes are already sorting toward the island cores that give them the biggest income edge. Alongside this, `world.hidingCount` was added — a cumulative counter of mote-ticks spent frozen in cover, incremented whenever `m._hiding` is `true` in `step()`. `observe.js` section [4] now reports "motes hid/1k", giving the concealment mechanic its first headless readout. First reading: 4.5–6.8/1k, proving cover IS being used despite the herd's elevated mean speed. **121 checks** (+6: four `sizeGrazeMult` invariant checks matching the new 0.8 range; two `world.hidingCount` checks — type and non-negativity).

Before that: **each mote now shows whether it is hiding or fleeing, tick by tick.** The flee/hide decision has always driven selection — the lifestyle rings around each mote show which *strategy* a mote's genome predisposes it to — but the *live choice each tick* was invisible. Now it isn't: `step()` caches two boolean flags per mote (`_threat`: a hunter was within fear range this tick; `_hiding`: the mote froze in cover rather than bolting), and `draw()` reads them. A **white center dot** marks a hiding mote — it froze in dense vegetation, invisible to distant hunters, a small pool of stillness in the herd. A **short red wake trail** (5.5px behind the mote, alpha 0.55) marks a fleeing mote — direction of panic sprint away from the threat, a smear of urgency. In practice the white dot is genuinely rare: in arms-race worlds the herd has evolved mean speed ~2.32, above `coverSpeedSeen = 2.2`, so virtually no mote can hide — speed literally breaks cover, and the flag is honest. The red wakes are common: 6–20% of motes per tick in a typical arms-race herd, a storm of flight responses that fades and rekindles with the hunt. Two new `smoke.js` assertions verify both flags are booleans after ticking — **115 checks** total.
