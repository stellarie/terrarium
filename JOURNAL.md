...
heritable `social` gene and boids-style flocking, meant to be a **third** anti-predator strategy beside
fleeing and hiding, paid for by predator DILUTION. The world **refused**, across five couplings (bare
dilution, a gated confusion effect, a fleeing murmuration, many-eyes vigilance, lock-on disruption):
gregariousness stayed flat-to-selected-**against** under predation, because the hunter is a **density-
seeker** — it homes on the nearest prey, so a crowd is a killing ground and grouping raises your
encounter rate faster than any confusion effect lowers your per-strike risk. So the gene's adaptive
direction is the **mirror image**: `social < 0` is **WARY** — it steers a mote away from local density.
Under predation the herd now evolves wary (measured **−0.44 … −0.76**, Δ vs predator-free **−0.5 … −0.7**,
robust across seeds 3/7/11/19 + unseeded); lift the hunters and it relaxes to ~0. A new regime-set axis,
parallel to hide↔flee and thrifty↔greedy, and one the divergence could someday split along. This is the
...
_Update (2026-08-18, the size-food Expedition — giving size an interior optimum, and reopening the hider niche):_ Grazers are still ONE broad cloud (k=1) on the morph detector — the arc's stubborn core holds — but two structural defects that had been silently blocking k=2 are now closed, and the first ecological response is visible in one 20k run.

**What was broken.** `observe.js` revealed two dead selection axes. (1) **Size was a pure cost with no income.** The bite rate (`vegGrazeRate=0.16`) was flat regardless of body size, but the metabolic cost scaled with `size×0.15` — so a large body paid more every tick and ate nothing extra. Natural selection eliminated large bodies; size slid to its floor (mean 2.05, histogram `[@=-:... ]`). (2) **No mote could hide.** Speed had drifted to 2.32 mean, above `coverSpeedSeen=2.2`, giving every mote `speedFact=0` and zero concealment regardless of cover. The hider niche was structurally disabled — `coverFreeze=0.55` was a threshold no mote could ever reach.

**What was built.** Three minimal changes, each targeting exactly one defect: (a) **`sizeGrazeMult(size)`** — a linear multiplier on bite rate, from 0.70× at min size (1.6) to 1.30× at max (6.5), neutral at the midpoint (4.05). Large bodies now earn 30% more food per bite; small bodies earn 30% less. (b) **`coverSpeedSeen: 2.2 → 3.0`** — at mean speed 2.32, `speedFact` is now `(3.0-2.32)/1.95 = 0.35` instead of 0; a mote in a good island cell reaches concealment ≈ 0.28–0.30. (c) **`coverFreeze: 0.55 → 0.30`** — motes in lush island cores can now actually freeze in cover (island max-hider cover reaches 0.77, well above the new threshold). Visual: mote bodies glow warm gold at the core scaled by body fullness, making the size-food tradeoff legible on the field. New `observe.js` report: size-quartile habitat split (does Q4/large preferentially occupy richer ground?). Four new `smoke.js` assertions verify `sizeGrazeMult` shape at min/mid/max and monotonicity — **119 checks**.

**What we see.** Across 4 seeds at 20k ticks: speed has already drifted **down** (seed 1: 0.97→2.17, was 2.32 in the previous field notes), and size has drifted **up** (seed 1: 3.36→2.50, was 2.05). Both directions are correct — the food-income advantage is pulling size upward; the hider niche is starting to pull speed down. Seed 7 shows **`SORTING`** — Q4(large) motes occupying richer ground than Q1(small) by +0.07 fertility units, exactly what you'd expect if large lineages are tracking the high-vegetation cells where their bite-rate bonus pays. K=1 still on every seed, but the two strategies are now structurally viable; the ecology just needs time to diverge.

_Update (2026-08-19, the size-gradient Build): size is now drifting UPWARD for the first time, and hiding is measurable._ Grazers still ONE broad cloud (k=1); this run didn't attempt the split. Observation found size still collapsing (3.33→2.06↓) despite the Expedition's `sizeGrazeMult` — because at strength=0.6, the small-mote bite rate (0.112) still exceeded typical sparse-cell availability (~0.098), so both sizes took the same bite and the metabolic-cost penalty on large bodies still won. At strength=0.9 the collapse was traded for a different problem: small-mote bite rate (0.088) dropped below typical availability, spiking starvation 9→44/1k and destabilising the hunter tier (53% collapse). The settled fix: `sizeGrazeStrength 0.6→0.8`, bringing small-mote bite rate to 0.096 — just at the tipping point where large motes gain a consistent advantage in typical cells without the drastic prey-base disruption. Result: in a fresh 15k-tick world, **size drifts UP (3.29→3.41 ↑)** for the first time, with **SORTING confirmed** (+0.06 fert gap, large motes on richer ground). Also added `world.hidingCount` — a per-tick accumulator of mote-ticks spent frozen in cover — reported as "motes hid/1k" in observe.js section [4] (first reading: **4.5–6.8/1k**, showing concealment IS being used). Two smoke assertions guard the counter. **121 checks**.

_Runs since the last Expedition:_ **1** — next Expedition mandatory after 4 more Builds.
...
**Last observed: 2026-08-19 — the size-gradient Build.** Readings: freely random 15k, seed 3 12k; `smoke.js` 121 checks green.

- **gene-pool shape:** grazers **k=1** (one broad cloud still); freely random 15k: speed 1.05→1.62↑, **size 3.29→3.41↑ ← UPWARD for first time**, sense 49→16↓ (collapsed-world artifact), metabo 1.05→1.39↑, social 0.08→0.01. Size histogram: `[...:-=+@@%%*=-:. ]` — mode shifting toward midrange. Hunters 15k: speed 1.44→2.03↑, size 4.40→5.48↑.
- **size-habitat sorting:** SORTING in freely random 15k (+0.06 fert gap, Q4(large) on richer ground); seed 3 gap +0.02 (weaker, collapsed world).
- **hiding events (new):** **4.5–6.8 mote-ticks/1k in cover**. The concealment mechanic is active; hiding was unmeasured before this run.
- **populations:** oscillating, no safety net activations. Matter HOLDING (0.0% drift). Freely random 15k: hunter collapse 52% of ticks (1 episode, recovering — 64 hunters at end); seed 3: 2 hunters at end (thin but present). Starvation elevated in collapsed-world seeds (52/1k vs 9/1k in healthy worlds).
- **smoke:** **121 checks** (+2 hidingCount assertions; sizeGrazeMult range updated 0.70→1.30 to 0.60→1.40).

_previously:_ (2026-08-18, size-food Expedition) k=1; speed 2.32, size 2.05 (at floor); zero hiding events (coverSpeedSeen 2.2 below mean speed → no hiding possible); smoke 119 checks.

_previously:_ (2026-08-17, behavioral-state Build) k=1; speed 2.32, size 2.05 (at floor); zero hiding events (coverSpeedSeen 2.2 below mean speed); smoke 115 checks.
...
- Core objects:
  ...
  - **hunter** (predator): `{ x, y, dir, energy, age, cool, homeX, homeY, g }` — `cool` is the digestion
    timer; `homeX/homeY` is the kill centroid (home-range EMA, added 2026-08-05).
...
- **The predation layer** (Arc II) — `world.hunters`, a second organism run right after the
  ...
  - **Hunter home ranges / kill centroids** (added 2026-08-05) — each hunter tracks `homeX, homeY`,
    an exponential moving average of its kill locations (initialised at spawn; updated with
    `hunterHomeShift=0.08` of each kill's toroidal vector). Every tick a gentle angular lean
    (`hunterHomePull=0.20`, scaled from 0 at home to full at `hunterHomeRange=220px`) steers the hunter
    toward its centroid — applied even while chasing, so geographic preference builds up in prey-rich
    environments. `draw()` renders a faint cross-hair at each centroid (`hsl(hue 55% 68% / 0.20)`) so
    the emerging territory structure is visible. `observe.js` gains `territoryMap()` — a 48×16 centroid
    density map and a mean nearest-neighbour distance vs. the uniform-random baseline. Territories emerge
    from habit, not decree; the mechanic is the arc's first spatial predation-pressure gradient.
...
### 2026-08-19 — [Build] strengthen size-food gradient and count hiding events

**Observed (the complaint):** Size was still sliding to the floor (mean 2.06, histogram `[@*=-:...]`) despite the Expedition's `sizeGrazeMult`. The cause: at `sizeGrazeStrength=0.6` the small-mote bite rate (0.112) still exceeded typical sparse-cell vegetation availability (~0.098), so both body sizes took the same bite and the metabolic-cost penalty on large bodies continued to win. The size advantage was real only in the 10% of island cells where veg exceeded the bite rate — not enough to overcome selection.

**Built.** `sizeGrazeStrength 0.6→0.8`, bringing small-mote bite rate to 0.096 — just below typical sparse-cell availability — so large motes earn consistently more food even outside island cores. (At 0.9, tested first, small-mote bite rate fell to 0.088, spiking starvation 9→44/1k and collapsing the hunter tier — too drastic. 0.8 is the tipping-point: large bodies gain in typical cells, small bodies are barely bite-rate-limited.) Also wired up `world.hidingCount` — a cumulative counter of mote-ticks spent frozen in cover — reported in `observe.js` section [4] as "motes hid/1k", guarded by 2 new smoke assertions. **121 checks** (+2).

**What the world now shows.** In a fresh 15k-tick world, size drifts **UP** (3.29→3.41 ↑) for the first time since the Expedition; size-habitat **SORTING** is confirmed (+0.06 fertility gap, large motes occupying richer ground). Before this run, size reliably sank toward 1.60 regardless of seed. Hiding is now measurable: **4.5–6.8 mote-ticks per 1k** spent frozen in island cover, proving the concealment mechanic is active. (Category: **ecology + instrumentation** — rotates off last run's visuals/ecology. Build; Expedition counter → **1**.)

### 2026-08-17 — [Build] show each mote's behavioral state: hiding gets a white dot, fleeing gets a red wake

**Observed (the complaint):** The world's central gameplay is the freeze-or-bolt decision a threatened mote makes every tick — stay put in cover (the hider's answer) or sprint away at panic speed (the fleer's). This decision is the arc's whole mechanism, and it was invisible. A mote choosing to freeze in dense island grass looked identical to one peacefully grazing; a mote bolting from a hunter looked like a mote that happened to be moving fast. Two behaviors that drive the entire ecology could not be told apart by watching.

**Built.** `step()` now caches two boolean flags on every mote: `m._threat` (a hunter was within fear range this tick) and `m._hiding` (the mote froze in cover rather than bolting). `draw()` reads them: a **hiding mote** — threat detected, chose to freeze — gets a small bright white center dot (the mote is locked down, barely twitching, invisible to distant hunters); a **fleeing mote** — threat detected, chose to bolt — gets a short **red wake trail** extending 5.5px behind it along its direction of travel, reading as the heat of panic flight. Motes grazing peacefully show neither. The flags are view-only (nothing in the economy reads them back) and the economy is byte-identical; two new smoke assertions confirm the flags are booleans after ticking. **115 checks** (+2).

**What a visitor now sees.** Scan the field during a predator surge: red wakes streak across the barrens as motes bolt in all directions, the trails pointing toward where they were and away from what scared them. In island grass, if a small/slow hider-genome mote is threatened and in dense enough cover (≥0.55 concealment), a white dot appears instead — the mote holding still, letting the hunter walk past. In the arms-race world observed, the fleer genome dominates (speed 2.32 > `coverSpeedSeen` 2.2 — no moving mote can hide) so red wakes fire for ~6–20% of the herd per tick and white dots are genuinely rare, a marker of a successful concealment. The rarity makes the white dot legible: each one is a genuine hider in the right place at the right time, not noise. (Category: **visuals** — rotates off last run's ecology/behavior. Build; Expedition counter → **4**.)
...
**Observed (the complaint):** `observe.js` showed 96 alarm sites all glowing the same uniform warm-red in `draw()`, regardless of which hunter made the kill. With 72–129 hunters each tracking a home centroid, the alarm layer was a featureless warm wash — you couldn't tell whose territory was whose from the colour. The cross-hair markers are the only territory markers, and they're very faint; the alarm glow is the dominant territorial signal, but it was saying nothing per-individual. Second finding (pre-existing defect, fixed this run): after a high-predation prey crash, kills slow to near-zero and the 96-site ring buffer stops refreshing — entries aged past `alarmDuration` and accumulated as stale sites that the draw code skipped but the smoke check flagged. This wasn't caused by any prior change; seed 7 exposed it (29 stale of 96 at run end).

**Built.** Two small, coupled changes. (1) **Per-hunter hue on kill log entries:** the `killLog.push(...)` call gains `hue: h.g.hue` — the killing hunter's hue gene (range 0–45, warm reds and oranges). The draw loop uses `hsla(${site.hue},72%,48%,${freshness * 0.055})` instead of the fixed `rgba(210,65,35,...)`, so each hunter's alarm sites glow in its personal shade — deep red for hue 5, amber-orange for hue 35. (2) **Stale-alarm trim in `step()`:** a single `while` at the end of the tick trims expired entries from the front of the insertion-ordered ring (entries older than `alarmDuration`). This keeps the ring to live entries at all times and resolves the smoke pre-existing failure — seed 7 now passes cleanly with 19 fresh sites instead of 96 with 29 stale. `smoke.js` gains 1 new assertion (every killLog entry carries a `hue` in [0,45]): **96 checks** total.

**What a visitor now sees.** Hunting grounds still glow warm and fade cool as predation ebbs, but the wash is no longer uniform: each hunter's kills pool in its own shade, so where two hunters' territories overlap the glow shows a mottled mix of hues, and a zone dominated by one hunter reads as a coherent colour. Over minutes of watching, territory structure that used to be readable only from the faint cross-hairs now legible directly in the alarm layer — a quick scan of which hue dominates which corner of the meadow tells you whose patrol ground it is. Economy and dynamics byte-identical (no `rng()`, no read-back); matter **HOLDING** (seed 3 8k). (Category: **visuals** — rotates off last run's ecology/behaviour. Build; Expedition counter → **1**.)

### 2026-08-05 — [Expedition] give each hunter a home range: kill centroids carve emergent territories

**Observed (the complaint):** `observe.js` section [8] (life map) showed hunters scattered uniformly across the torus — no clustering, no corridors, no hot zones. With predation spatially uniform every patch of meadow carries the same predation pressure, so the lifestyle axis (hide-small vs flee-fast) can only pick a *global* winner per world rather than running both simultaneously in different zones. Arc III's within-world coexistence requires at least the first step: hunters who *go home* to where they've been successful, carving personal patrol corridors out of the shared landscape. A second complaint: `alarmSites = 12` at ~1.85 kills/tick meant the ring buffer cycled every ~6 ticks — the kill-site alarm was functionally useless as spatial memory, all 12 sites perpetually fresh (flagged in the 08-04 log entry as the next lever).

**Built (two coupled mechanics).** (1) **Widen the kill-site alarm:** `alarmSites` raised 12→96, giving the ring buffer ~52 ticks of kill history instead of ~6, so the alarm glow can accumulate into a real spatial footprint — territory cores glow, refugia stay dark. (2) **Per-hunter kill centroids:** each hunter now tracks `homeX, homeY`, an exponential moving-average of its kill sites (initialised at spawn). After every kill the centroid shifts `hunterHomeShift = 0.08` of the way toward the kill location (toroidal shortest-path math throughout). Every tick the hunter applies a gentle angular lean (`hunterHomePull = 0.20`) toward its centroid, scaled from 0 at home to full at `hunterHomeRange = 220px` distance — applied even while chasing, so geographic preference accumulates in prey-rich environments. Three new CONFIG knobs; `makeHunter` gains `homeX/homeY`. `draw()` places a faint `hsl(hue 55% 68% / 0.20)` cross-hair at each hunter's kill centroid so the territory structure is visible at a glance. `smoke.js` gains 2 assertions (all home centroids finite and in-canvas). `observe.js` gains `territoryMap()` — a 48×16 ASCII density map of home centroids plus a mean nearest-neighbour statistic vs. the uniform-random baseline.

**What a visitor now sees.** Each predator has a small, faint cross-hair drifting toward where it kills most — gradually, over minutes of watching, each hunter's cross-hair settles into its patrol corridor while the alarm glows pool under it. The territories aren't walls; they're habits. **Observatory reading (seed 3, 8k):** mean NN 43.6px vs. uniform-random baseline ~84.3px — centroids cluster rather than spread, because seed 3 crashed to 3 motes by tick 8000 and all hunters converged on the same scarce prey. Territory *separation* needs a high-prey-density world; this run delivered the mechanism, and the census will show whether separation emerges in the richer seeds. `smoke.js` **95 checks** (+2 home-centroid proofs), green ×3; matter **HOLDING** (+0.9%/0.2% — within normal sample noise at 8k). (Category: **ecology/behaviour** — rotates off last run's same category but this is the mandatory Expedition. Expedition counter → **0**; next Expedition mandatory after 5 Builds.)
...
**Observed (the complaint):** `social` is the strongest-drifting gene in the world (0.16 → −0.48 unseeded
— the biggest Δ in the boredom check by far), the `herd` chip names its current value live, but the trait
chart still had only three lines (speed, size, sense). A visitor could read the wariness *now* but couldn't
watch it *evolve* — couldn't see the line dive as a cull builds and ease back in a lull, the way they can
follow speed's arms-race climb. The most compelling story in the history buffer had no curve. The
split-test also surfaced a new finding: **2/3 no-hunter worlds show within-world 2-morph coexistence**
(seed 1 along `speed`, seed 3 along `sense`) — crowding-driven speciation, first time visible paired
alongside the predation-driven wariness result.

**Built.** `social` now flows into `world.history` each sample and draws as a fourth grazer line — **cool
blue** (matching the wary whisker), normalized over [−1, 1.2] — on the trait chart. The legend handles it
cleanly: it has no hunter counterpart, so it shows as `social −0.45` without the `·hunter` pair, and the
trailing key reads "grazer·hunter (social: grazer)" to stay honest. One new smoke check confirms every
history sample carries a finite in-range `social` mean.

**What a visitor now sees.** Open the world and the cool blue line traces how the herd learned to keep its
distance: it starts near zero with the founding generation, dives toward −0.5 as predation presses, relaxes
in a lull. The chip reads *now*; the chart reads *how it got there*. Economy untouched; `smoke.js` **89
checks** (+1), green ×3. (Category: **dataviz** — rotates off last run's UI/instrumentation. Expedition
counter → **2**.)
...
**Observed (the complaint):** the herd is behaviourally one-dimensional. After ~15 runs it has exactly two
answers to a hunter — flee and hide — whose single optimum each world settles on, which is *why* every
divergence attempt averages out; grazers are still ONE broad cloud (k=1, seed 3 shows only the old `size`
BC⚑ hint). And a fresh unseeded low-predation draw had `sense` **pinned at its floor (⚑LO, 13.75)** — with
no threat to detect and food easy, every grazer collapses onto the same cheap generalist. The world needed
a genuinely new axis to act on. This was a mandatory-Expedition run (5th since the nutrient cycle) and the
category rotation was overdue for ecology/behaviour, so I built one.

**Built — and the reversal that named it.** A sixth heritable gene, `social`, and an O(n) neighbour grid
(a spatial hash, reusable infra). The plan was **"safety in numbers":** motes flock, and predator DILUTION
makes grouping a *third* anti-predator strategy. The world refused. I tested **five couplings** — bare
dilution, a gated confusion effect, a fleeing murmuration, many-eyes vigilance, and lock-on disruption —
and every one left gregariousness flat-to-selected-**against** under predation, because the hunter homes on
the *nearest* prey: a crowd is a killing ground, and grouping raises your encounter rate faster than any
confusion effect lowers your per-strike risk (forcing full schooling only starved the hunters to collapse).
So I flipped the gene: `social < 0` is **WARY** — it steers a mote *away* from local density. That the world
selects **hard**: under predation the herd evolves **−0.44 … −0.76** (Δ vs predator-free −0.5…−0.7, robust
across seeds 3/7/11/19 + unseeded); remove the hunters and it relaxes to ~0. I then stripped all the refuted
benefit machinery, so the wariness is the world's own honest verdict, not a tuned artifact.

**What a visitor now sees.** Motes react to each other for the first time — each trails a **heading whisker
tinted by sociability** (cool blue = wary, keeps its distance; warm orange = sociable; pale = neutral), and a
soft **density shimmer** glows on the herd's packed knots — which, in this world, are the risky crowds the
hunters cull, so you can watch them thin as the herd turns wary under predation and pool again in a lull.
`observe.js` reports the new axis in [6]/[9] with a wary/sociable verdict and a **clumping index**. Honest
limit: the gene is *quiet* in the raw texture (resource/flee clustering still packs the herd, index ~1.9–5×);
the strong signal lives in the gene mean, and the tint/readout are what make it legible.

**Verified.** No exceptions/NaN; populations oscillate healthily (motes ~455, hunters ~85, plants ~437,
unseeded); matter **HOLDING** (−0.1%); no gene newly pinned; `smoke.js` **87 checks** (+4 sociability proofs)
green ×5 seeds; `--frame` renders; zero runtime network calls. Neighbour query ~doubled tick cost (24s/20k).
...
- **[Build] Retired (BUILT 2026-08-01): "Put `social` on the trait chart."** Done: `social` now flows into
  `world.history` each sample and draws as a fourth grazer line — cool blue, [−1, 1.2] — with a clean
...
- **[Build] Retired (BUILT 2026-08-19): "Count hiding events in observe.js and smoke: at end of each tick record whether any mote was hiding (m._hiding), and surface it in [4] FLOW alongside the killed/starved/aged tallies."** Done: `world.hidingCount` counter added; observe.js section [4] reports "motes hid/1k"; smoke checks type and non-negativity. First reading: 4.5–6.8/1k, proving the concealment mechanic fires. Smoke 121/121.

- **[Build] Island-affinity check for hiding events — `world.hidingOnIsland`.** Instrument whether hiding is actually happening in the island cores. Add `world.hidingOnIsland = 0` alongside `world.hidingCount` in world init; increment it in `step()` when `hiding && world.fert[y * COLS + x] >= 0.75`. In observe.js section [4], report the island share of all hiding events (`hidingOnIsland / max(hidingCount, 1)`). If the share stays > 70% the concealment mechanic routes motes to island cover as designed; near 15% (the island footprint) means hiding is opportunistic and not island-driven. Instrumentation-only; no economy change. Acceptance: observe.js section [4] shows the island-hiding share; smoke checks `typeof world.hidingOnIsland === 'number'`.

- **[Expedition] Size-by-lifestyle split: do larger motes hide more and smaller ones flee?** _(ambitious — only ~5 motes are hiding at any given tick, so the mean size of hiders is noisy; a rolling accumulator is needed, and distinguishing "large hides more" from "large motes happen to be in island cores which happen to have cover" requires a control)_. The income gradient (0.8 strength) routes large motes to islands; the concealment mechanic fires in cover. If both are working together, `meanSize(hiding) > meanSize(fleeing)` should hold by a detectable margin across seeds. Instrument: in each `step()` tick, accumulate `hidingSizeSum`, `hidingSizeN`, `fleeingSizeSum`, `fleeingSizeN` as world globals; maintain a 300-tick rolling window. In observe.js section [7] SHAPE report "mean size — hiding: X.X | fleeing: X.X". A gap > 0.3 consistent across seeds is the first direct evidence that two lifestyle clusters are forming even while k stays 1. If absent, the income gradient is not yet strong enough to spatially separate strategies. Risk: the hiding pool (~5/tick) is genuinely small; the rolling mean helps, but a 6-seed × 7200-tick split-test is likely needed to read a real signal above noise.
- **[Expedition] Run the size-food split-test — does `sizeGrazeMult` finally crack k=2?** _(ambitious — the size-food Expedition gave large bodies an income advantage and reopened the hider niche; now measure whether two morphs actually form under predation)._ Run `observe.js --split-test 6 20000` and check the shape section: does the morph detector read k=2 in any seed along size or speed? Note whether the SORTING flag (Q4(large) on richer ground) predicts which seeds tip toward k=2. If even 1/6 splits, it is the first confirmed within-world coexistence under predation driven by the food-tradeoff alone. If none split, check whether bc on size is rising (the valley forming slowly). Next lever if it stays k=1: `sizeGrazeStrength 0.6 → 0.9` (stronger income gradient), or a direct body-armor escape probability for large motes (a second form of predation resistance besides concealment, making the large strategy more viable under heavy predation). Risk: the seed-dependent SORTING signal is promising but 20k may not be long enough for two clusters to fully separate — try 40k on candidate seeds.
- **[Expedition] Body-armor escape: large motes get a chance to shrug off a strike** _(ambitious — adds a second predation-resistance channel for large morphs, distinct from concealment, potentially the key that makes k=2 stable under heavy predation)._ When a hunter's strike succeeds, if `prey.g.size ≥ sizeArmorMin` (≈3.5), roll `rng() < armorEscape(size)` where `armorEscape` scales from 0 at min-armor to 0.25 at max size. On escape: mote survives, loses 25% energy (shock), hunter still resets its cooldown (wasted strike). This gives large motes a survival advantage that doesn't depend on vegetation density or speed — it fires in the open, unlike concealment — creating a genuine SECOND viable large-mote strategy orthogonal to hiding. The selection pressure: small-fast motes escape via speed (flee) or concealment (hide in grass); large-slow motes escape via armor (survive the hit). Medium motes fail at all three — the fitness valley that locks in k=2. Needs `rng()` draw inside the hunter strike block (reproducible via seeded PRNG). Risk: 25% escape probability may not be enough to overcome the large mote's higher catch-range (`prey.g.size` adds to `cr`), and too-strong armor (>40%) may suppress predation to the point of hunter collapse — needs careful smoke testing and a watch on predation share. Currently `_hiding` is set each tick but never aggregated — observe.js can't report how many hiding events occurred over a run. A simple `world.hidden++` counter (incremented for each `m._hiding = true` mote each tick) would give the per-1k hiding rate, making the concealment mechanic measurable headlessly for the first time. Acceptance: observe.js section [4] shows a "motes hid/1k" line; smoke checks `typeof world.hidden === 'number'`. Small, bounded, instrumentation-only.

- **[Expedition] Make the behavioral state ring SPATIALLY legible: an overlay mode that shades cells by current hiding vs fleeing density.** The new `_hiding`/`_threat` flags fire per-mote per-tick — aggregate them into a per-cell grid (how many motes are currently hiding here? how many bolting?) and paint it as a third behavioral overlay mode (next to fertility and grazing). Island cores in a live arms-race world would show clusters of hiding dots in the dense cover; the barrens would show red-wake density from motes bolting across open ground. This makes the spatial structure of the hide/flee strategy landscape visible at a landscape scale, not just per-mote — the instrument the arc needs to confirm that island cover is genuinely bifurcating the herd's behavior by location. _(Ambitious — the overlay must update each frame from the current mote array without perturbing the economy, and the cell grid must be smoothed/accumulated to avoid single-mote noise.)_
...
- **[Expedition] Chase the within-world splits the split-test just revealed: crowding forces speciation
  ...
  2/3 predator-free worlds show within-world 2-morph coexistence (speed in seed 1, sense in seed 3) — the
  crowding mechanism delivers it reliably without predators. The puzzle is why predation _suppresses_ it:
  predation drives a strong between-world divergence (fast fleers vs slow hiders) but prevents the within-
  world split that crowding alone achieves. Hypothesis: predation homogenises the herd under one shared
  optimal lifestyle (each world tips its predation regime first, which sets a single optimal answer before
  crowding pressure can carve two). Test: can **spatial food heterogeneity** (dense-cover cores + open
  barrens, permanently coexisting on one map) sustain BOTH crowding-driven and predation-driven pressure
  simultaneously, so the `classifyMorphs` detector finally reads k=2 WITH hunters? The split-test is already
  the right rig — check whether `--seed 1` (which splits on `speed` without hunters) also splits with them
  under a patchy-fertility map. Risk: the spatial structure averages out again; the regime may still win.
...
- **[Expedition] Make predation SPATIALLY patchy so the wary axis SPLITS within a world — a density-seeking
  ...
  (wary under predation) but stays ONE cloud (k=1). Give it a reason to fracture: make the hunter a *visible*
  density-seeker — steer it up the local prey-density gradient, not just at the nearest mote — so crowds are
  causally, watchably lethal. Then in **cover-rich refuges** a small hider freezes and wariness is pointless
  (you're hidden, not fleeing), while in the **open** wariness is the whole defence — two habitats, two
  optima on `social` at once. Check `classifyMorphs` for a stable **k=2 along `social`** that collapses when
  the density preference is switched off. Note for a future session: the *dilution/"safety in numbers"*
  framing was tried five ways this run and is **refuted** — the hunter's density-seeking is exactly why
  grouping never pays, so lean into it as the splitting force, don't fight it. Risk: over-harvest crashes
  the tuned limit cycle, or spatial structure averages out yet again; needs its own smoke assertions and a
  careful watch on the pyramid.
...
- **[Build] Retired (BUILT 2026-08-05): "Widen the kill-site alarm memory to 96 sites."** Done: `alarmSites`
  raised 12→96, giving ~52 ticks of kill history instead of ~6. Alarm sites now span a real age range (1–563t,
  freshness 0.06–1.00 in observe.js), so the danger glow accumulates into a genuine spatial footprint.

- **[Expedition] Retired (BUILT 2026-08-05): "Give each hunter a home range via kill centroids."** Done:
  per-hunter `homeX/homeY` (kill EMA) + home-pull steering every tick + territory cross-hair in draw() +
  `territoryMap()` in observe.js + 2 smoke assertions. Mean NN 43.6px vs uniform ~84.3px on seed 3 at 8k —
  clustering confirms the centroid mechanic is live; territory *separation* needs a higher-prey-density read.

- **[Build] Map territory overlap onto the alarm glow — use each hunter's home centroid to colour its alarm
  contribution.** Currently all kill-site alarm sites glow the same warm-red regardless of which hunter made
  the kill. Tag each `killLog` entry with the killer's hue and use it to tint the glow — a warm-amber core
  for a fast orange hunter, a cooler red-violet core for another — so overlapping territories visually separate
  and a visitor can tell at a glance whether one hunter dominates a zone or two are competing for it. Bounded
  (killLog entry gets a new `hue` field; alarm glow reads it), view-only, no economy read-back.
...
- **[Expedition] Run the paired split-test under the new territory mechanic — does spatial predation pressure
  finally produce within-world k=2?** _(ambitious — every k=2-under-predation attempt has failed; this is the
  first time the landscape of danger is set by hunter habit, not random walk)_. Now that hunters carve patrol
  corridors, the herd may experience meaningfully different selection pressure depending on where they graze —
  a hider in a refuge, a fleer in the open. Run `observe.js --split-test 6 20000` and check the shape section:
  does the morph detector read k=2 in any seed? Does the split collapse when `hunterHomePull` is zeroed (the
  "territory off" condition)? If yes, this is the first confirmed within-world coexistence and the arc is
  substantially advanced. Risk: territories may not be spatially sharp enough to sustain two optima, or the NN
  distance (43.6px observed) may indicate hunters cluster near prey rather than spread, so the "danger gradient"
  is still spatially homogeneous from the herd's perspective.
