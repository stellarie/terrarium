...
_Runs since the last Expedition:_ **0** — size-food Expedition (2026-08-18). **Next Expedition mandatory after 5 more Builds.**
...
**Last observed: 2026-08-18 — the size-food Expedition.** Readings: `observe.js` seeds 1/7/42/99 ×20k; `smoke.js` 119 checks green.

- **gene-pool shape:** grazers **k=1** (one broad cloud still); seed 1: speed 0.97→2.17↑ (down from 2.32 baseline — hider pull beginning), size 3.36→2.50↑ (up from 2.05 floor — food-income advantage), sense 46→54↑, metabo 0.97→1.60↑; social 0.08→-0.64 (wary). Hunters seed 1: speed/size/sense/metabo all rising.
- **size-habitat sorting (new):** seed 7 shows Q4(large) on richer ground than Q1(small) by +0.07 fert ← SORTING; seed 1 gap -0.01 (not sorted yet); seed 42 -0.04. Signal emerging, not universal yet.
- **concealment:** island max-hider cover ≈ 0.77 (well above coverFreeze=0.30) — genuine hiders CAN freeze in island cover now. Barren max-hider cover ≈ 0.24 (below threshold) — fleeing in the open.
- **populations:** all oscillating, no safety net activations across 4 seeds. 96% predation deaths. Matter HOLDING.
- **smoke:** **119 checks** (+4 sizeGrazeMult shape assertions: min 0.70, mid 1.00, max 1.30, monotone).
...
- **[Build] Count hiding events in observe.js and smoke: at end of each tick record whether any mote was hiding (m._hiding), and surface it in [4] FLOW alongside the killed/starved/aged tallies.**
